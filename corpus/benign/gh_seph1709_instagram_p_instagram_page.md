---
name: instagram-page
description: "Requires: powershell/pwsh. Reads ~/.config/instagram-page/credentials.json (IG_ACCESS_TOKEN, IG_USER_ID). Interact with any Instagram Business/Creator account via the Instagram Graph API (graph.facebook.com only) - post photos, carousels, reels, stories, get insights, read and reply to comments, moderate. IG_APP_SECRET for one-time OAuth2 setup only - delete afterward. IG_ACCESS_TOKEN expires every 60 days - refresh before expiry. Grant token minimal permissions only. No data forwarded to third parties; all calls go to graph.facebook.com only."
