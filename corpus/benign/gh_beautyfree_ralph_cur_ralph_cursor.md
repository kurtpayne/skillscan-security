...
license: MIT
compatibility: Designed for Cursor (nightly). Requires Node.js 18+, cursor-agent CLI, git.
metadata:
...
Use the Node CLI (`npx ralph-cursor` or `ralph-cursor`) for init, run, loop, once, task, doctor, status, and logs.
