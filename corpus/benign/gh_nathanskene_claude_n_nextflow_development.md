...
## nf-core Config Gotchas (HPC)

### base.config overrides your profile defaults for unnamed processes

nf-core pipelines ship `conf/base.config` which sets default resources (typically 6GB/1CPU). These defaults apply to ANY process not explicitly overridden with a `withName` block in your custom config. Your profile's `process { memory = '16.GB' }` is overridden by base.config for named processes.

**Fix:** Use generous defaults with attempt-based scaling and extended retries:
...
This auto-heals OOM failures (16→32→48GB) without needing to predict which processes will fail.

### BWA_INDEX runs even when using bwamem2 aligner

nf-core/raredisease builds standard BWA indexes for mitochondrial alignment even when bwamem2 is the primary aligner. Configure resources for ALL index processes, not just BWAMEM2_INDEX.

### nf-core ships an `imperial` profile

For Imperial HPC, use `-profile imperial` which auto-configures PBS Pro. Combine with your custom config for resource overrides.
